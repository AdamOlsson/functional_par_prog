...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_1d *read_value_4260;
    int64_t read_shape_4261[1];
    int32_t *read_arr_4262 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4262, read_shape_4261, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0,
                      "[]", i32_info.type_name, strerror(errno));
    
    struct futhark_bool_1d *read_value_4263;
    int64_t read_shape_4264[1];
    bool *read_arr_4265 = NULL;
    
    errno = 0;
    if (read_array(&bool_info, (void **) &read_arr_4265, read_shape_4264, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1,
    ...
    struct futhark_i32_1d *result_4266;
    
    if (perform_warmup) {
        int r;
        
        assert((read_value_4260 = futhark_new_i32_1d(ctx, read_arr_4262,
                                                     read_shape_4261[0])) != 0);
        assert((read_value_4263 = futhark_new_bool_1d(ctx, read_arr_4265,
                                                      read_shape_4264[0])) !=
            0);
        ...
        r = futhark_entry_main(ctx, &result_4266, read_value_4260,
                               read_value_4263);
        if (r != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4260) == 0);
        assert(futhark_free_bool_1d(ctx, read_value_4263) == 0);
        assert(futhark_free_i32_1d(ctx, result_4266) == 0);
    }
    time_runs = 1;
    // Proper run.
        ...
        set_binary_mode(stdout);
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_i32_1d(ctx, result_4266, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_1d(ctx, result_4266), 1);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_i32_1d(ctx, result_4266) == 0);
...
static const char *opencl_program[] =
                  {"#ifdef cl_clang_storage_class_specifiers\n#pragma OPENCL EXTENSION cl_clang_storage_class_specifiers : enable\n#endif\n#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n__kernel void dummy_kernel(__global unsigned char *dummy, int n)\n{\n    const int thread_gid = get_global_id(0);\n    \n    if (thread_gid >= n)\n        return;\n}\ntypedef char int8_t;\ntypedef short int16_t;\ntypedef int int32_t;\ntypedef long int64_t;\ntypedef uchar uint8_t;\ntypedef ushort uint16_t;\ntypedef uint uint32_t;\ntypedef ulong uint64_t;\n#ifdef cl_nv_pragma_unroll\nstatic inline void mem_fence_global()\n{\n    asm(\"membar.gl;\");\n}\n#else\nstatic inline void mem_fence_global()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n}\n#endif\nstatic inline void mem_fence_local()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE);\n}\nstatic inline uint8_t add8(uint8_t x, uint8_t y)\n{\n    return x + y;\n}\nstatic inline uint16_t add16(uint16_t x, uint16_t y)\n{\n    return x + y;\n}\nstatic inline uint32_t add32(uint32_t x, uint32_t y)\n{\n    return x + y;\n}\nstatic inline uint64_t add64(uint64_t x, uint64_t y)\n{\n    return x + y;\n}\nstatic inline uint8_t sub8(uint8_t x, uint8_t y)\n{\n    return x - y;\n}\nstatic inline uint16_t sub16(uint16_t x, uint16_t y)\n{\n    return x - y;\n}\nstatic inline uint32_t sub32(uint32_t x, uint32_t y)\n{\n    return x - y;\n}\nstatic inline uint64_t sub64(uint64_t x, uint64_t y)\n{\n    return x - y;\n}\nstatic inline uint8_t mul8(uint8_t x, uint8_t y)\n{\n    return x * y;\n}\nstatic inline uint16_t mul16(uint16_t x, uint16_t y)\n{\n    return x * y;\n}\nstatic inline uint32_t mul32(uint32_t x, uint32_t y)\n{\n    return x * y;\n}\nstatic inline uint64_t mul64(uint64_t x, uint64_t y)\n{\n    return x * y;\n}\nstatic inline uint8_t udiv8(uint8_t x, uint8_t y)\n{\n    return x / y;\n}\nstatic inline uint16_t udiv16(uint16_t x, uint16_t y)\n{\n    return x / y;\n}\nstatic inline uint32_t udiv32(uint32_t x, uint32_t y)\n{\n    return x / y;\n}\nstatic inline uint64_t udiv64(uint64_t x, uint64_t y)\n{\n    return x / y;\n}\nstatic ",
                   ...
                   ", int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicOr((int32_t*)p, x);\n#else\n  return atomic_or(p, x);\n#endif\n}\n\ninline int32_t atomic_xor_i32_global(volatile __global int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicXor((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xor_i32_local(volatile __local int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicXor((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xchg_i32_global(volatile __global int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicExch((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xchg_i32_local(volatile __local int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicExch((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_cmpxchg_i32_global(volatile __global int32_t *p,\n                                         int32_t cmp, int32_t val) {\n#ifdef FUTHARK_CUDA\n  return atomicCAS((int32_t*)p, cmp, val);\n#else\n  return atomic_cmpxchg(p, cmp, val);\n#endif\n}\n\ninline int32_t atomic_cmpxchg_i32_local(volatile __local int32_t *p,\n                                         int32_t cmp, int32_t val) {\n#ifdef FUTHARK_CUDA\n  return atomicCAS((int32_t*)p, cmp, val);\n#else\n  return atomic_cmpxchg(p, cmp, val);\n#endif\n}\n\n// End of atomics.h\n\n__kernel void scan_stage1_4178(__global int *global_failure, __local volatile\n                               int64_t *scan_arr_mem_4200_backing_aligned_0,\n                               int32_t n_4161, __global\n                               unsigned char *k_mem_4180, __global\n                               unsigned char *mem_4185,\n                               int32_t num_threads_4191)\n{\n    #define segscan_group_sizze_4173 (mainzisegscan_group_sizze_4172)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    __local volatile char *restrict scan_arr_mem_4200_backing_0 =\n               ",
                   "           (__local volatile\n                           char *) scan_arr_mem_4200_backing_aligned_0;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4195;\n    int32_t local_tid_4196;\n    int32_t group_sizze_4199;\n    int32_t wave_sizze_4198;\n    int32_t group_tid_4197;\n    \n    global_tid_4195 = get_global_id(0);\n    local_tid_4196 = get_local_id(0);\n    group_sizze_4199 = get_local_size(0);\n    wave_sizze_4198 = LOCKSTEP_WIDTH;\n    group_tid_4197 = get_group_id(0);\n    \n    int32_t phys_tid_4178 = global_tid_4195;\n    __local char *scan_arr_mem_4200;\n    \n    scan_arr_mem_4200 = (__local char *) scan_arr_mem_4200_backing_0;\n    \n    int32_t x_4166;\n    int32_t x_4167;\n    \n    x_4166 = 0;\n    for (int32_t j_4202 = 0; j_4202 < squot32(sub32(add32(n_4161,\n                                                          num_threads_4191), 1),\n                                              num_threads_4191); j_4202++) {\n        int32_t chunk_offset_4203 = add32(mul32(segscan_group_sizze_4173,\n                                                j_4202), mul32(group_tid_4197,\n                                                               mul32(segscan_group_sizze_4173,\n                                                                     squot32(sub32(add32(n_4161,\n                                                                                         num_threads_4191),\n                                                                                   1),\n                                                                             num_threads_4191))));\n        int32_t flat_idx_4204 = add32(chunk_offset_4203, local_tid_4196);\n        int32_t gtid_4177 = flat_idx_4204;\n        \n        // threads in bounds read input; others get neutral element\n        {\n            if (slt32(gtid_4177, n_4161)) {\n                int32_t x_4169 = ((__global int32_t *) k_mem_4180)[gtid_4177];\n                \n                // write to-scan values to parameters\n         ",
                   "       {\n                    x_4167 = x_4169;\n                }\n                // write mapped values results to global memory\n                { }\n            } else {\n                x_4167 = 0;\n            }\n        }\n        // combine with carry and write to local memory\n        {\n            int32_t res_4168 = add32(x_4166, x_4167);\n            \n            ((__local int32_t *) scan_arr_mem_4200)[local_tid_4196] = res_4168;\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        \n        int32_t x_4192;\n        int32_t x_4193;\n        int32_t x_4205;\n        int32_t x_4206;\n        int32_t skip_threads_4208;\n        \n        if (slt32(local_tid_4196, segscan_group_sizze_4173)) {\n            x_4193 = ((volatile __local\n                       int32_t *) scan_arr_mem_4200)[local_tid_4196];\n            if (sub32(local_tid_4196, mul32(squot32(local_tid_4196, 32), 32)) ==\n                0) {\n                x_4192 = x_4193;\n            }\n        }\n        // in-block scan (hopefully no barriers needed)\n        {\n            skip_threads_4208 = 1;\n            while (slt32(skip_threads_4208, 32)) {\n                if (sle32(skip_threads_4208, sub32(local_tid_4196,\n                                                   mul32(squot32(local_tid_4196,\n                                                                 32), 32))) &&\n                    slt32(local_tid_4196, segscan_group_sizze_4173)) {\n                    // read operands\n                    {\n                        x_4192 = ((volatile __local\n                                   int32_t *) scan_arr_mem_4200)[sub32(local_tid_4196,\n                                                                       skip_threads_4208)];\n                    }\n                    // perform operation\n                    {\n                        int32_t res_4194 = add32(x_4192, x_4193);\n                        \n                        x_4192 = res_4194;\n                    }\n                }\n                if (sle32(wave_sizze_4",
                   "198, skip_threads_4208)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                if (sle32(skip_threads_4208, sub32(local_tid_4196,\n                                                   mul32(squot32(local_tid_4196,\n                                                                 32), 32))) &&\n                    slt32(local_tid_4196, segscan_group_sizze_4173)) {\n                    // write result\n                    {\n                        ((volatile __local\n                          int32_t *) scan_arr_mem_4200)[local_tid_4196] =\n                            x_4192;\n                        x_4193 = x_4192;\n                    }\n                }\n                if (sle32(wave_sizze_4198, skip_threads_4208)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                skip_threads_4208 *= 2;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // last thread of block 'i' writes its result to offset 'i'\n        {\n            if (sub32(local_tid_4196, mul32(squot32(local_tid_4196, 32), 32)) ==\n                31 && slt32(local_tid_4196, segscan_group_sizze_4173)) {\n                ((volatile __local\n                  int32_t *) scan_arr_mem_4200)[squot32(local_tid_4196, 32)] =\n                    x_4192;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // scan the first block, after which offset 'i' contains carry-in for block 'i+1'\n        {\n            int32_t skip_threads_4209;\n            \n            if (squot32(local_tid_4196, 32) == 0 && slt32(local_tid_4196,\n                                                          segscan_group_sizze_4173)) {\n                x_4206 = ((volatile __local\n                           int32_t *) scan_arr_mem_4200)[local_tid_4196];\n                if (sub32(local_tid_4196, mul32(squot32(local_tid_4196, 32),\n                                                32)) == 0) {\n                    x_4205 = x_4206;\n                }\n            }\n      ",
                   "      // in-block scan (hopefully no barriers needed)\n            {\n                skip_threads_4209 = 1;\n                while (slt32(skip_threads_4209, 32)) {\n                    if (sle32(skip_threads_4209, sub32(local_tid_4196,\n                                                       mul32(squot32(local_tid_4196,\n                                                                     32),\n                                                             32))) &&\n                        (squot32(local_tid_4196, 32) == 0 &&\n                         slt32(local_tid_4196, segscan_group_sizze_4173))) {\n                        // read operands\n                        {\n                            x_4205 = ((volatile __local\n                                       int32_t *) scan_arr_mem_4200)[sub32(local_tid_4196,\n                                                                           skip_threads_4209)];\n                        }\n                        // perform operation\n                        {\n                            int32_t res_4207 = add32(x_4205, x_4206);\n                            \n                            x_4205 = res_4207;\n                        }\n                    }\n                    if (sle32(wave_sizze_4198, skip_threads_4209)) {\n                        barrier(CLK_LOCAL_MEM_FENCE);\n                    }\n                    if (sle32(skip_threads_4209, sub32(local_tid_4196,\n                                                       mul32(squot32(local_tid_4196,\n                                                                     32),\n                                                             32))) &&\n                        (squot32(local_tid_4196, 32) == 0 &&\n                         slt32(local_tid_4196, segscan_group_sizze_4173))) {\n                        // write result\n                        {\n                            ((volatile __local\n                              int32_t *) scan_arr_mem_4200)[local_tid_4196] =\n                         ",
                   "       x_4205;\n                            x_4206 = x_4205;\n                        }\n                    }\n                    if (sle32(wave_sizze_4198, skip_threads_4209)) {\n                        barrier(CLK_LOCAL_MEM_FENCE);\n                    }\n                    skip_threads_4209 *= 2;\n                }\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // carry-in for every block except the first\n        {\n            if (!(squot32(local_tid_4196, 32) == 0 || !slt32(local_tid_4196,\n                                                             segscan_group_sizze_4173))) {\n                // read operands\n                {\n                    x_4193 = x_4192;\n                    x_4192 = ((__local\n                               int32_t *) scan_arr_mem_4200)[sub32(squot32(local_tid_4196,\n                                                                           32),\n                                                                   1)];\n                }\n                // perform operation\n                {\n                    int32_t res_4194 = add32(x_4192, x_4193);\n                    \n                    x_4192 = res_4194;\n                }\n                // write final result\n                {\n                    ((__local int32_t *) scan_arr_mem_4200)[local_tid_4196] =\n                        x_4192;\n                }\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // restore correct values for first block\n        {\n            if (squot32(local_tid_4196, 32) == 0) {\n                ((__local int32_t *) scan_arr_mem_4200)[local_tid_4196] =\n                    x_4193;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // threads in bounds write partial scan result\n        {\n            if (slt32(gtid_4177, n_4161)) {\n                ((__global int32_t *) mem_4185)[gtid_4177] = ((__local\n                                                               int32_t *) scan_arr_mem_4200)[local_tid_4196",
                   "];\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // first thread reads last element as carry-in for next iteration\n        {\n            bool crosses_segment_4210 = 0;\n            bool should_load_carry_4211 = local_tid_4196 == 0 &&\n                 !crosses_segment_4210;\n            \n            if (should_load_carry_4211) {\n                x_4166 = ((__local\n                           int32_t *) scan_arr_mem_4200)[sub32(segscan_group_sizze_4173,\n                                                               1)];\n            }\n            if (!should_load_carry_4211) {\n                x_4166 = 0;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n    }\n    \n  error_1:\n    return;\n    #undef segscan_group_sizze_4173\n}\n__kernel void scan_stage2_4178(__global int *global_failure, __local volatile\n                               int64_t *scan_arr_mem_4223_backing_aligned_0,\n                               int32_t n_4161, int32_t num_groups_4175, __global\n                               unsigned char *mem_4185,\n                               int32_t num_threads_4191)\n{\n    #define segscan_group_sizze_4173 (mainzisegscan_group_sizze_4172)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    __local volatile char *restrict scan_arr_mem_4223_backing_0 =\n                          (__local volatile\n                           char *) scan_arr_mem_4223_backing_aligned_0;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4218;\n    int32_t local_tid_4219;\n    int32_t group_sizze_4222;\n    int32_t wave_sizze_4221;\n    int32_t group_tid_4220;\n    \n    global_tid_4218 = get_global_id(0);\n    local_tid_4219 = get_local_id(0);\n    group_sizze_4222 = get_local_size(0);\n    wave_sizze_4221 = LOCKSTEP_WIDTH;\n    group_tid_4220 = get_group_id(0);\n    \n    int32_t phys_tid_4178 = global_tid_4218;\n    __local char *scan_arr_mem_4223;\n    \n    scan_arr_mem_4223 = (__local char *) scan_",
                   "arr_mem_4223_backing_0;\n    \n    int32_t flat_idx_4225 = sub32(mul32(add32(local_tid_4219, 1),\n                                        mul32(segscan_group_sizze_4173,\n                                              squot32(sub32(add32(n_4161,\n                                                                  num_threads_4191),\n                                                            1),\n                                                      num_threads_4191))), 1);\n    int32_t gtid_4177 = flat_idx_4225;\n    \n    // threads in bound read carries; others get neutral element\n    {\n        if (slt32(gtid_4177, n_4161)) {\n            ((__local int32_t *) scan_arr_mem_4223)[local_tid_4219] = ((__global\n                                                                        int32_t *) mem_4185)[gtid_4177];\n        } else {\n            ((__local int32_t *) scan_arr_mem_4223)[local_tid_4219] = 0;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    \n    int32_t x_4212;\n    int32_t x_4213;\n    int32_t x_4226;\n    int32_t x_4227;\n    int32_t skip_threads_4229;\n    \n    if (slt32(local_tid_4219, num_groups_4175)) {\n        x_4213 = ((volatile __local\n                   int32_t *) scan_arr_mem_4223)[local_tid_4219];\n        if (sub32(local_tid_4219, mul32(squot32(local_tid_4219, 32), 32)) ==\n            0) {\n            x_4212 = x_4213;\n        }\n    }\n    // in-block scan (hopefully no barriers needed)\n    {\n        skip_threads_4229 = 1;\n        while (slt32(skip_threads_4229, 32)) {\n            if (sle32(skip_threads_4229, sub32(local_tid_4219,\n                                               mul32(squot32(local_tid_4219,\n                                                             32), 32))) &&\n                slt32(local_tid_4219, num_groups_4175)) {\n                // read operands\n                {\n                    x_4212 = ((volatile __local\n                               int32_t *) scan_arr_mem_4223)[sub32(local_tid_4219,\n                                               ",
                   "                    skip_threads_4229)];\n                }\n                // perform operation\n                {\n                    int32_t res_4214 = add32(x_4212, x_4213);\n                    \n                    x_4212 = res_4214;\n                }\n            }\n            if (sle32(wave_sizze_4221, skip_threads_4229)) {\n                barrier(CLK_LOCAL_MEM_FENCE);\n            }\n            if (sle32(skip_threads_4229, sub32(local_tid_4219,\n                                               mul32(squot32(local_tid_4219,\n                                                             32), 32))) &&\n                slt32(local_tid_4219, num_groups_4175)) {\n                // write result\n                {\n                    ((volatile __local\n                      int32_t *) scan_arr_mem_4223)[local_tid_4219] = x_4212;\n                    x_4213 = x_4212;\n                }\n            }\n            if (sle32(wave_sizze_4221, skip_threads_4229)) {\n                barrier(CLK_LOCAL_MEM_FENCE);\n            }\n            skip_threads_4229 *= 2;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // last thread of block 'i' writes its result to offset 'i'\n    {\n        if (sub32(local_tid_4219, mul32(squot32(local_tid_4219, 32), 32)) ==\n            31 && slt32(local_tid_4219, num_groups_4175)) {\n            ((volatile __local\n              int32_t *) scan_arr_mem_4223)[squot32(local_tid_4219, 32)] =\n                x_4212;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // scan the first block, after which offset 'i' contains carry-in for block 'i+1'\n    {\n        int32_t skip_threads_4230;\n        \n        if (squot32(local_tid_4219, 32) == 0 && slt32(local_tid_4219,\n                                                      num_groups_4175)) {\n            x_4227 = ((volatile __local\n                       int32_t *) scan_arr_mem_4223)[local_tid_4219];\n            if (sub32(local_tid_4219, mul32(squot32(local_tid_4219, 32), 32)) ==\n                0) {\n                ",
                   "x_4226 = x_4227;\n            }\n        }\n        // in-block scan (hopefully no barriers needed)\n        {\n            skip_threads_4230 = 1;\n            while (slt32(skip_threads_4230, 32)) {\n                if (sle32(skip_threads_4230, sub32(local_tid_4219,\n                                                   mul32(squot32(local_tid_4219,\n                                                                 32), 32))) &&\n                    (squot32(local_tid_4219, 32) == 0 && slt32(local_tid_4219,\n                                                               num_groups_4175))) {\n                    // read operands\n                    {\n                        x_4226 = ((volatile __local\n                                   int32_t *) scan_arr_mem_4223)[sub32(local_tid_4219,\n                                                                       skip_threads_4230)];\n                    }\n                    // perform operation\n                    {\n                        int32_t res_4228 = add32(x_4226, x_4227);\n                        \n                        x_4226 = res_4228;\n                    }\n                }\n                if (sle32(wave_sizze_4221, skip_threads_4230)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                if (sle32(skip_threads_4230, sub32(local_tid_4219,\n                                                   mul32(squot32(local_tid_4219,\n                                                                 32), 32))) &&\n                    (squot32(local_tid_4219, 32) == 0 && slt32(local_tid_4219,\n                                                               num_groups_4175))) {\n                    // write result\n                    {\n                        ((volatile __local\n                          int32_t *) scan_arr_mem_4223)[local_tid_4219] =\n                            x_4226;\n                        x_4227 = x_4226;\n                    }\n                }\n                if (sle32(wave_sizze_4221, skip_threa",
                   "ds_4230)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                skip_threads_4230 *= 2;\n            }\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // carry-in for every block except the first\n    {\n        if (!(squot32(local_tid_4219, 32) == 0 || !slt32(local_tid_4219,\n                                                         num_groups_4175))) {\n            // read operands\n            {\n                x_4213 = x_4212;\n                x_4212 = ((__local\n                           int32_t *) scan_arr_mem_4223)[sub32(squot32(local_tid_4219,\n                                                                       32), 1)];\n            }\n            // perform operation\n            {\n                int32_t res_4214 = add32(x_4212, x_4213);\n                \n                x_4212 = res_4214;\n            }\n            // write final result\n            {\n                ((__local int32_t *) scan_arr_mem_4223)[local_tid_4219] =\n                    x_4212;\n            }\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // restore correct values for first block\n    {\n        if (squot32(local_tid_4219, 32) == 0) {\n            ((__local int32_t *) scan_arr_mem_4223)[local_tid_4219] = x_4213;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // threads in bounds write scanned carries\n    {\n        if (slt32(gtid_4177, n_4161)) {\n            ((__global int32_t *) mem_4185)[gtid_4177] = ((__local\n                                                           int32_t *) scan_arr_mem_4223)[local_tid_4219];\n        }\n    }\n    \n  error_0:\n    return;\n    #undef segscan_group_sizze_4173\n}\n__kernel void scan_stage3_4178(__global int *global_failure, int32_t n_4161,\n                               int32_t num_groups_4175, __global\n                               unsigned char *mem_4185,\n                               int32_t num_threads_4191,\n                               int32_t required_groups_4231)\n{\n    #define segscan_group_sizze_4173 (",
                   "mainzisegscan_group_sizze_4172)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4232;\n    int32_t local_tid_4233;\n    int32_t group_sizze_4236;\n    int32_t wave_sizze_4235;\n    int32_t group_tid_4234;\n    \n    global_tid_4232 = get_global_id(0);\n    local_tid_4233 = get_local_id(0);\n    group_sizze_4236 = get_local_size(0);\n    wave_sizze_4235 = LOCKSTEP_WIDTH;\n    group_tid_4234 = get_group_id(0);\n    \n    int32_t phys_tid_4178 = global_tid_4232;\n    int32_t phys_group_id_4237;\n    \n    phys_group_id_4237 = get_group_id(0);\n    for (int32_t i_4238 = 0; i_4238 <\n         squot32(sub32(add32(sub32(required_groups_4231, phys_group_id_4237),\n                             num_groups_4175), 1), num_groups_4175); i_4238++) {\n        int32_t virt_group_id_4239 = add32(phys_group_id_4237, mul32(i_4238,\n                                                                     num_groups_4175));\n        int32_t flat_idx_4240 = add32(mul32(virt_group_id_4239,\n                                            segscan_group_sizze_4173),\n                                      local_tid_4233);\n        int32_t gtid_4177 = flat_idx_4240;\n        int32_t orig_group_4241 = squot32(flat_idx_4240,\n                                          mul32(segscan_group_sizze_4173,\n                                                squot32(sub32(add32(n_4161,\n                                                                    num_threads_4191),\n                                                              1),\n                                                        num_threads_4191)));\n        int32_t carry_in_flat_idx_4242 = sub32(mul32(orig_group_4241,\n                                                     mul32(segscan_group_sizze_4173,\n                                                           squot32(sub32(add32(n_4161,\n                                                                     ",
                   "          num_threads_4191),\n                                                                         1),\n                                                                   num_threads_4191))),\n                                               1);\n        \n        if (slt32(gtid_4177, n_4161)) {\n            if (!(orig_group_4241 == 0 || flat_idx_4240 ==\n                  sub32(mul32(add32(orig_group_4241, 1),\n                              mul32(segscan_group_sizze_4173,\n                                    squot32(sub32(add32(n_4161,\n                                                        num_threads_4191), 1),\n                                            num_threads_4191))), 1))) {\n                int32_t x_4215;\n                int32_t x_4216;\n                \n                x_4215 = ((__global\n                           int32_t *) mem_4185)[carry_in_flat_idx_4242];\n                x_4216 = ((__global int32_t *) mem_4185)[gtid_4177];\n                \n                int32_t res_4217 = add32(x_4215, x_4216);\n                \n                x_4215 = res_4217;\n                ((__global int32_t *) mem_4185)[gtid_4177] = x_4215;\n            }\n        }\n        barrier(CLK_GLOBAL_MEM_FENCE);\n    }\n    \n  error_0:\n    return;\n    #undef segscan_group_sizze_4173\n}\n",
                   NULL};
static const char *size_names[] = {"main.segscan_group_size_4172",
                                   "main.segscan_num_groups_4174"};
static const char *size_vars[] = {"mainzisegscan_group_sizze_4172",
                                  "mainzisegscan_num_groups_4174"};
...
struct sizes {
    size_t mainzisegscan_group_sizze_4172;
    size_t mainzisegscan_num_groups_4174;
} ;
struct futhark_context_config {
...
struct futhark_context {
    ...
    int total_runs;
    long total_runtime;
    cl_kernel scan_stage1_4178;
    cl_kernel scan_stage2_4178;
    cl_kernel scan_stage3_4178;
    ...
    int64_t scan_stage1_4178_total_runtime;
    int scan_stage1_4178_runs;
    int64_t scan_stage2_4178_total_runtime;
    int scan_stage2_4178_runs;
    int64_t scan_stage3_4178_total_runtime;
    int scan_stage3_4178_runs;
                               ...
                               struct futhark_context *ctx)
{
    ...
    ctx->scan_stage1_4178_total_runtime = 0;
    ctx->scan_stage1_4178_runs = 0;
    ctx->scan_stage2_4178_total_runtime = 0;
    ctx->scan_stage2_4178_runs = 0;
    ctx->scan_stage3_4178_total_runtime = 0;
    ctx->scan_stage3_4178_runs = 0;
}
                             ...
                             struct futhark_context *ctx, cl_program prog)
{
    ...
    OPENCL_SUCCEED_OR_RETURN(error);
    {
        ctx->scan_stage1_4178 = clCreateKernel(prog, "scan_stage1_4178",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage1_4178, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
    ...
    {
        ctx->scan_stage2_4178 = clCreateKernel(prog, "scan_stage2_4178",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage2_4178, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "scan_stage2_4178");
    ...
    {
        ctx->scan_stage3_4178 = clCreateKernel(prog, "scan_stage3_4178",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage3_4178, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "scan_stage3_4178");
    }
    ctx->sizes.mainzisegscan_group_sizze_4172 = cfg->sizes[0];
    ctx->sizes.mainzisegscan_num_groups_4174 = cfg->sizes[1];
    init_constants(ctx);
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock_device *out_mem_p_4243,
                       int32_t *out_out_arrsizze_4244,
                       struct memblock_device k_mem_4180,
                       struct memblock_device d_mem_4181, int32_t n_4161,
                       int32_t n_4162);
...
void futhark_debugging_report(struct futhark_context *ctx)
{
    ...
    if (ctx->profiling) {
        ...
        fprintf(stderr,
                "scan_stage1_4178     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage1_4178_runs,
                (long) ctx->scan_stage1_4178_total_runtime /
                (ctx->scan_stage1_4178_runs !=
                 0 ? ctx->scan_stage1_4178_runs : 1),
                (long) ctx->scan_stage1_4178_total_runtime);
        ctx->total_runtime += ctx->scan_stage1_4178_total_runtime;
        ctx->total_runs += ctx->scan_stage1_4178_runs;
        fprintf(stderr,
                "scan_stage2_4178     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage2_4178_runs,
                (long) ctx->scan_stage2_4178_total_runtime /
                (ctx->scan_stage2_4178_runs !=
                 0 ? ctx->scan_stage2_4178_runs : 1),
                (long) ctx->scan_stage2_4178_total_runtime);
        ctx->total_runtime += ctx->scan_stage2_4178_total_runtime;
        ctx->total_runs += ctx->scan_stage2_4178_runs;
        fprintf(stderr,
                "scan_stage3_4178     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage3_4178_runs,
                (long) ctx->scan_stage3_4178_total_runtime /
                (ctx->scan_stage3_4178_runs !=
                 0 ? ctx->scan_stage3_4178_runs : 1),
                (long) ctx->scan_stage3_4178_total_runtime);
        ctx->total_runtime += ctx->scan_stage3_4178_total_runtime;
        ctx->total_runs += ctx->scan_stage3_4178_runs;
        if (ctx->profiling)
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       int32_t n_4162)
{
    struct memblock_device out_mem_4188;
    
    out_mem_4188.references = NULL;
    
    int32_t out_arrsizze_4189;
    int64_t n_4170 = sext_i32_i64(n_4161);
    int32_t segscan_group_sizze_4173;
    
    segscan_group_sizze_4173 = ctx->sizes.mainzisegscan_group_sizze_4172;
    
    int32_t num_groups_4175;
    int32_t max_num_groups_4190;
    
    max_num_groups_4190 = ctx->sizes.mainzisegscan_num_groups_4174;
    num_groups_4175 = sext_i64_i32(smax64(1, smin64(squot64(sub64(add64(n_4170,
                                                                        sext_i32_i64(segscan_group_sizze_4173)),
                                                                  1),
                                                            sext_i32_i64(segscan_group_sizze_4173)),
                                                    sext_i32_i64(max_num_groups_4190))));
    
    int64_t bytes_4183 = mul64(4, n_4170);
    struct memblock_device mem_4185;
    
    mem_4185.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4185, bytes_4183, "mem_4185"))
        return 1;
    if (ctx->debugging)
        fprintf(stderr, "%s\n", "\n# SegScan");
    
    int32_t num_threads_4191 = mul32(num_groups_4175, segscan_group_sizze_4173);
    
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4178, 1,
                                            mul32((int32_t) sizeof(int32_t),
                                                  segscan_group_sizze_4173),
                                            NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4178, 2,
                                            sizeof(n_4161), &n_4161));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4178, 3,
                                            sizeof(k_mem_4180.mem),
                                            &k_mem_4180.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4178, 4,
                                            sizeof(mem_4185.mem),
                                            &mem_4185.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4178, 5,
                                            sizeof(num_threads_4191),
                                            &num_threads_4191));
    if (1 * (num_groups_4175 * segscan_group_sizze_4173) != 0) {
        const size_t global_work_sizze_4245[1] = {num_groups_4175 *
                     segscan_group_sizze_4173};
        const size_t local_work_sizze_4249[1] = {segscan_group_sizze_4173};
        int64_t time_start_4246 = 0, time_end_4247 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage1_4178");
            fprintf(stderr, "%zu", global_work_sizze_4245[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4249[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) (0 + mul32((int32_t) sizeof(int32_t),
                                     segscan_group_sizze_4173)));
            time_start_4246 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage1_4178,
                                                        1, NULL,
                                                        global_work_sizze_4245,
                                                        local_work_sizze_4249,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage1_4178_runs,
                                                                                                  &ctx->scan_stage1_4178_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4247 = get_wall_time();
            
            long time_diff_4248 = time_end_4247 - time_start_4246;
    ...
    if (ctx->debugging)
        fprintf(stderr, "%s: %llu%c", "elems_per_group",
                (long long) mul32(segscan_group_sizze_4173,
                                  squot32(sub32(add32(n_4161, num_threads_4191),
                                                1), num_threads_4191)), '\n');
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4178, 1,
                                            mul32((int32_t) sizeof(int32_t),
                                                  num_groups_4175), NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4178, 2,
                                            sizeof(n_4161), &n_4161));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4178, 3,
                                            sizeof(num_groups_4175),
                                            &num_groups_4175));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4178, 4,
                                            sizeof(mem_4185.mem),
                                            &mem_4185.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4178, 5,
                                            sizeof(num_threads_4191),
                                            &num_threads_4191));
    if (1 * (1 * num_groups_4175) != 0) {
        const size_t global_work_sizze_4250[1] = {1 * num_groups_4175};
        const size_t local_work_sizze_4254[1] = {num_groups_4175};
        int64_t time_start_4251 = 0, time_end_4252 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage2_4178");
            fprintf(stderr, "%zu", global_work_sizze_4250[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4254[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) (0 + mul32((int32_t) sizeof(int32_t),
                                     num_groups_4175)));
            time_start_4251 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage2_4178,
                                                        1, NULL,
                                                        global_work_sizze_4250,
                                                        local_work_sizze_4254,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage2_4178_runs,
                                                                                                  &ctx->scan_stage2_4178_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4252 = get_wall_time();
            
            long time_diff_4253 = time_end_4252 - time_start_4251;
    ...
    int32_t required_groups_4231 = squot32(sub32(add32(n_4161,
                                                       segscan_group_sizze_4173),
                                                 1), segscan_group_sizze_4173);
    
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4178, 1,
                                            sizeof(n_4161), &n_4161));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4178, 2,
                                            sizeof(num_groups_4175),
                                            &num_groups_4175));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4178, 3,
                                            sizeof(mem_4185.mem),
                                            &mem_4185.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4178, 4,
                                            sizeof(num_threads_4191),
                                            &num_threads_4191));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4178, 5,
                                            sizeof(required_groups_4231),
                                            &required_groups_4231));
    if (1 * (num_groups_4175 * segscan_group_sizze_4173) != 0) {
        const size_t global_work_sizze_4255[1] = {num_groups_4175 *
                     segscan_group_sizze_4173};
        const size_t local_work_sizze_4259[1] = {segscan_group_sizze_4173};
        int64_t time_start_4256 = 0, time_end_4257 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage3_4178");
            fprintf(stderr, "%zu", global_work_sizze_4255[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4259[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) 0);
            time_start_4256 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage3_4178,
                                                        1, NULL,
                                                        global_work_sizze_4255,
                                                        local_work_sizze_4259,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage3_4178_runs,
                                                                                                  &ctx->scan_stage3_4178_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4257 = get_wall_time();
            
            long time_diff_4258 = time_end_4257 - time_start_4256;
            
            fprintf(stderr, "kernel %s runtime: %ldus\n", "scan_stage3_4178",
                    time_diff_4258);
        }
    }
    out_arrsizze_4189 = n_4161;
    if (memblock_set_device(ctx, &out_mem_4188, &mem_4185, "mem_4185") != 0)
        return 1;
    (*out_mem_p_4243).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4243, &out_mem_4188,
                            "out_mem_4188") != 0)
        return 1;
    *out_out_arrsizze_4244 = out_arrsizze_4189;
    if (memblock_unref_device(ctx, &mem_4185, "mem_4185") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4188, "out_mem_4188") != 0)
        return 1;
    return 0;
}
struct futhark_bool_1d {
                       ...
                       struct futhark_bool_1d *in1)
{
    ...
    k_mem_4180.references = NULL;
    
    struct memblock_device d_mem_4181;
    
    d_mem_4181.references = NULL;
    ...
    int ret = futrts_main(ctx, &out_mem_4188, &out_arrsizze_4189, k_mem_4180,
                          d_mem_4181, n_4161, n_4162);
    
    if (ret == 0) {
        assert((*out0 =
                (struct futhark_i32_1d *) malloc(sizeof(struct futhark_i32_1d))) !=
            NULL);
        (*out0)->mem = out_mem_4188;
        (*out0)->shape[0] = out_arrsizze_4189;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
