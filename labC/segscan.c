...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_1d *read_value_4608;
    int64_t read_shape_4609[1];
    int32_t *read_arr_4610 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4610, read_shape_4609, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0,
                      "[]", i32_info.type_name, strerror(errno));
    
    struct futhark_bool_1d *read_value_4611;
    int64_t read_shape_4612[1];
    bool *read_arr_4613 = NULL;
    
    errno = 0;
    if (read_array(&bool_info, (void **) &read_arr_4613, read_shape_4612, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1,
    ...
    struct futhark_i32_1d *result_4614;
    
    if (perform_warmup) {
        int r;
        
        assert((read_value_4608 = futhark_new_i32_1d(ctx, read_arr_4610,
                                                     read_shape_4609[0])) != 0);
        assert((read_value_4611 = futhark_new_bool_1d(ctx, read_arr_4613,
                                                      read_shape_4612[0])) !=
            0);
        ...
        r = futhark_entry_main(ctx, &result_4614, read_value_4608,
                               read_value_4611);
        if (r != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4608) == 0);
        assert(futhark_free_bool_1d(ctx, read_value_4611) == 0);
        assert(futhark_free_i32_1d(ctx, result_4614) == 0);
    }
    time_runs = 1;
    // Proper run.
        ...
        set_binary_mode(stdout);
    {
        ...
        assert(arr != NULL);
        assert(futhark_values_i32_1d(ctx, result_4614, arr) == 0);
        write_array(stdout, binary_output, &i32_info, arr,
                    futhark_shape_i32_1d(ctx, result_4614), 1);
        free(arr);
    }
    printf("\n");
    assert(futhark_free_i32_1d(ctx, result_4614) == 0);
...
static const char *opencl_program[] =
                  {"#ifdef cl_clang_storage_class_specifiers\n#pragma OPENCL EXTENSION cl_clang_storage_class_specifiers : enable\n#endif\n#pragma OPENCL EXTENSION cl_khr_byte_addressable_store : enable\n__kernel void dummy_kernel(__global unsigned char *dummy, int n)\n{\n    const int thread_gid = get_global_id(0);\n    \n    if (thread_gid >= n)\n        return;\n}\ntypedef char int8_t;\ntypedef short int16_t;\ntypedef int int32_t;\ntypedef long int64_t;\ntypedef uchar uint8_t;\ntypedef ushort uint16_t;\ntypedef uint uint32_t;\ntypedef ulong uint64_t;\n#ifdef cl_nv_pragma_unroll\nstatic inline void mem_fence_global()\n{\n    asm(\"membar.gl;\");\n}\n#else\nstatic inline void mem_fence_global()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);\n}\n#endif\nstatic inline void mem_fence_local()\n{\n    mem_fence(CLK_LOCAL_MEM_FENCE);\n}\nstatic inline uint8_t add8(uint8_t x, uint8_t y)\n{\n    return x + y;\n}\nstatic inline uint16_t add16(uint16_t x, uint16_t y)\n{\n    return x + y;\n}\nstatic inline uint32_t add32(uint32_t x, uint32_t y)\n{\n    return x + y;\n}\nstatic inline uint64_t add64(uint64_t x, uint64_t y)\n{\n    return x + y;\n}\nstatic inline uint8_t sub8(uint8_t x, uint8_t y)\n{\n    return x - y;\n}\nstatic inline uint16_t sub16(uint16_t x, uint16_t y)\n{\n    return x - y;\n}\nstatic inline uint32_t sub32(uint32_t x, uint32_t y)\n{\n    return x - y;\n}\nstatic inline uint64_t sub64(uint64_t x, uint64_t y)\n{\n    return x - y;\n}\nstatic inline uint8_t mul8(uint8_t x, uint8_t y)\n{\n    return x * y;\n}\nstatic inline uint16_t mul16(uint16_t x, uint16_t y)\n{\n    return x * y;\n}\nstatic inline uint32_t mul32(uint32_t x, uint32_t y)\n{\n    return x * y;\n}\nstatic inline uint64_t mul64(uint64_t x, uint64_t y)\n{\n    return x * y;\n}\nstatic inline uint8_t udiv8(uint8_t x, uint8_t y)\n{\n    return x / y;\n}\nstatic inline uint16_t udiv16(uint16_t x, uint16_t y)\n{\n    return x / y;\n}\nstatic inline uint32_t udiv32(uint32_t x, uint32_t y)\n{\n    return x / y;\n}\nstatic inline uint64_t udiv64(uint64_t x, uint64_t y)\n{\n    return x / y;\n}\nstatic ",
                   ...
                   ", int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicOr((int32_t*)p, x);\n#else\n  return atomic_or(p, x);\n#endif\n}\n\ninline int32_t atomic_xor_i32_global(volatile __global int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicXor((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xor_i32_local(volatile __local int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicXor((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xchg_i32_global(volatile __global int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicExch((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_xchg_i32_local(volatile __local int32_t *p, int32_t x) {\n#ifdef FUTHARK_CUDA\n  return atomicExch((int32_t*)p, x);\n#else\n  return atomic_xor(p, x);\n#endif\n}\n\ninline int32_t atomic_cmpxchg_i32_global(volatile __global int32_t *p,\n                                         int32_t cmp, int32_t val) {\n#ifdef FUTHARK_CUDA\n  return atomicCAS((int32_t*)p, cmp, val);\n#else\n  return atomic_cmpxchg(p, cmp, val);\n#endif\n}\n\ninline int32_t atomic_cmpxchg_i32_local(volatile __local int32_t *p,\n                                         int32_t cmp, int32_t val) {\n#ifdef FUTHARK_CUDA\n  return atomicCAS((int32_t*)p, cmp, val);\n#else\n  return atomic_cmpxchg(p, cmp, val);\n#endif\n}\n\n// End of atomics.h\n\n__kernel void scan_stage1_4490(__global int *global_failure, __local volatile\n                               int64_t *scan_arr_mem_4522_backing_aligned_0,\n                               __local volatile\n                               int64_t *scan_arr_mem_4520_backing_aligned_1,\n                               int32_t n_4462, __global\n                               unsigned char *k_mem_4492, __global\n                               unsigned char *d_mem_4493, __global\n                               unsigned char *mem_4497, __global\n                               unsigned char *mem_4499,\n                               int32_t num_threa",
                   "ds_4505)\n{\n    #define segscan_group_sizze_4485 (mainzisegscan_group_sizze_4484)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    __local volatile char *restrict scan_arr_mem_4522_backing_1 =\n                          (__local volatile\n                           char *) scan_arr_mem_4522_backing_aligned_0;\n    __local volatile char *restrict scan_arr_mem_4520_backing_0 =\n                          (__local volatile\n                           char *) scan_arr_mem_4520_backing_aligned_1;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4515;\n    int32_t local_tid_4516;\n    int32_t group_sizze_4519;\n    int32_t wave_sizze_4518;\n    int32_t group_tid_4517;\n    \n    global_tid_4515 = get_global_id(0);\n    local_tid_4516 = get_local_id(0);\n    group_sizze_4519 = get_local_size(0);\n    wave_sizze_4518 = LOCKSTEP_WIDTH;\n    group_tid_4517 = get_group_id(0);\n    \n    int32_t phys_tid_4490 = global_tid_4515;\n    __local char *scan_arr_mem_4520;\n    \n    scan_arr_mem_4520 = (__local char *) scan_arr_mem_4520_backing_0;\n    \n    __local char *scan_arr_mem_4522;\n    \n    scan_arr_mem_4522 = (__local char *) scan_arr_mem_4522_backing_1;\n    \n    int32_t x_4471;\n    bool x_4472;\n    int32_t x_4473;\n    bool x_4474;\n    \n    x_4471 = 0;\n    x_4472 = 0;\n    for (int32_t j_4524 = 0; j_4524 < squot32(sub32(add32(n_4462,\n                                                          num_threads_4505), 1),\n                                              num_threads_4505); j_4524++) {\n        int32_t chunk_offset_4525 = add32(mul32(segscan_group_sizze_4485,\n                                                j_4524), mul32(group_tid_4517,\n                                                               mul32(segscan_group_sizze_4485,\n                                                                     squot32(sub32(add32(n_4462,\n                                                                                         num_th",
                   "reads_4505),\n                                                                                   1),\n                                                                             num_threads_4505))));\n        int32_t flat_idx_4526 = add32(chunk_offset_4525, local_tid_4516);\n        int32_t gtid_4489 = flat_idx_4526;\n        \n        // threads in bounds read input; others get neutral element\n        {\n            if (slt32(gtid_4489, n_4462)) {\n                int32_t x_4480 = ((__global int32_t *) k_mem_4492)[gtid_4489];\n                bool x_4481 = ((__global bool *) d_mem_4493)[gtid_4489];\n                \n                // write to-scan values to parameters\n                {\n                    x_4473 = x_4480;\n                    x_4474 = x_4481;\n                }\n                // write mapped values results to global memory\n                { }\n            } else {\n                x_4473 = 0;\n                x_4474 = 0;\n            }\n        }\n        // combine with carry and write to local memory\n        {\n            int32_t res_4475;\n            bool res_4476;\n            \n            if (x_4474) {\n                bool res_4477 = x_4472 || x_4474;\n                \n                res_4475 = x_4473;\n                res_4476 = res_4477;\n            } else {\n                int32_t res_4478 = add32(x_4471, x_4473);\n                bool res_4479 = x_4472 || x_4474;\n                \n                res_4475 = res_4478;\n                res_4476 = res_4479;\n            }\n            ((__local int32_t *) scan_arr_mem_4520)[local_tid_4516] = res_4475;\n            ((__local bool *) scan_arr_mem_4522)[local_tid_4516] = res_4476;\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        \n        int32_t x_4506;\n        bool x_4507;\n        int32_t x_4508;\n        bool x_4509;\n        int32_t x_4527;\n        bool x_4528;\n        int32_t x_4529;\n        bool x_4530;\n        int32_t skip_threads_4536;\n        \n        if (slt32(local_tid_4516, segscan_group_sizze_4485)) {",
                   "\n            x_4508 = ((volatile __local\n                       int32_t *) scan_arr_mem_4520)[local_tid_4516];\n            x_4509 = ((volatile __local\n                       bool *) scan_arr_mem_4522)[local_tid_4516];\n            if (sub32(local_tid_4516, mul32(squot32(local_tid_4516, 32), 32)) ==\n                0) {\n                x_4506 = x_4508;\n                x_4507 = x_4509;\n            }\n        }\n        // in-block scan (hopefully no barriers needed)\n        {\n            skip_threads_4536 = 1;\n            while (slt32(skip_threads_4536, 32)) {\n                if (sle32(skip_threads_4536, sub32(local_tid_4516,\n                                                   mul32(squot32(local_tid_4516,\n                                                                 32), 32))) &&\n                    slt32(local_tid_4516, segscan_group_sizze_4485)) {\n                    // read operands\n                    {\n                        x_4506 = ((volatile __local\n                                   int32_t *) scan_arr_mem_4520)[sub32(local_tid_4516,\n                                                                       skip_threads_4536)];\n                        x_4507 = ((volatile __local\n                                   bool *) scan_arr_mem_4522)[sub32(local_tid_4516,\n                                                                    skip_threads_4536)];\n                    }\n                    // perform operation\n                    {\n                        int32_t res_4510;\n                        bool res_4511;\n                        \n                        if (x_4509) {\n                            bool res_4512 = x_4507 || x_4509;\n                            \n                            res_4510 = x_4508;\n                            res_4511 = res_4512;\n                        } else {\n                            int32_t res_4513 = add32(x_4506, x_4508);\n                            bool res_4514 = x_4507 || x_4509;\n                            \n               ",
                   "             res_4510 = res_4513;\n                            res_4511 = res_4514;\n                        }\n                        x_4506 = res_4510;\n                        x_4507 = res_4511;\n                    }\n                }\n                if (sle32(wave_sizze_4518, skip_threads_4536)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                if (sle32(skip_threads_4536, sub32(local_tid_4516,\n                                                   mul32(squot32(local_tid_4516,\n                                                                 32), 32))) &&\n                    slt32(local_tid_4516, segscan_group_sizze_4485)) {\n                    // write result\n                    {\n                        ((volatile __local\n                          int32_t *) scan_arr_mem_4520)[local_tid_4516] =\n                            x_4506;\n                        x_4508 = x_4506;\n                        ((volatile __local\n                          bool *) scan_arr_mem_4522)[local_tid_4516] = x_4507;\n                        x_4509 = x_4507;\n                    }\n                }\n                if (sle32(wave_sizze_4518, skip_threads_4536)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                skip_threads_4536 *= 2;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // last thread of block 'i' writes its result to offset 'i'\n        {\n            if (sub32(local_tid_4516, mul32(squot32(local_tid_4516, 32), 32)) ==\n                31 && slt32(local_tid_4516, segscan_group_sizze_4485)) {\n                ((volatile __local\n                  int32_t *) scan_arr_mem_4520)[squot32(local_tid_4516, 32)] =\n                    x_4506;\n                ((volatile __local\n                  bool *) scan_arr_mem_4522)[squot32(local_tid_4516, 32)] =\n                    x_4507;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // scan the first block, after which offset 'i' contains carr",
                   "y-in for block 'i+1'\n        {\n            int32_t skip_threads_4537;\n            \n            if (squot32(local_tid_4516, 32) == 0 && slt32(local_tid_4516,\n                                                          segscan_group_sizze_4485)) {\n                x_4529 = ((volatile __local\n                           int32_t *) scan_arr_mem_4520)[local_tid_4516];\n                x_4530 = ((volatile __local\n                           bool *) scan_arr_mem_4522)[local_tid_4516];\n                if (sub32(local_tid_4516, mul32(squot32(local_tid_4516, 32),\n                                                32)) == 0) {\n                    x_4527 = x_4529;\n                    x_4528 = x_4530;\n                }\n            }\n            // in-block scan (hopefully no barriers needed)\n            {\n                skip_threads_4537 = 1;\n                while (slt32(skip_threads_4537, 32)) {\n                    if (sle32(skip_threads_4537, sub32(local_tid_4516,\n                                                       mul32(squot32(local_tid_4516,\n                                                                     32),\n                                                             32))) &&\n                        (squot32(local_tid_4516, 32) == 0 &&\n                         slt32(local_tid_4516, segscan_group_sizze_4485))) {\n                        // read operands\n                        {\n                            x_4527 = ((volatile __local\n                                       int32_t *) scan_arr_mem_4520)[sub32(local_tid_4516,\n                                                                           skip_threads_4537)];\n                            x_4528 = ((volatile __local\n                                       bool *) scan_arr_mem_4522)[sub32(local_tid_4516,\n                                                                        skip_threads_4537)];\n                        }\n                        // perform operation\n                        {\n                            i",
                   "nt32_t res_4531;\n                            bool res_4532;\n                            \n                            if (x_4530) {\n                                bool res_4533 = x_4528 || x_4530;\n                                \n                                res_4531 = x_4529;\n                                res_4532 = res_4533;\n                            } else {\n                                int32_t res_4534 = add32(x_4527, x_4529);\n                                bool res_4535 = x_4528 || x_4530;\n                                \n                                res_4531 = res_4534;\n                                res_4532 = res_4535;\n                            }\n                            x_4527 = res_4531;\n                            x_4528 = res_4532;\n                        }\n                    }\n                    if (sle32(wave_sizze_4518, skip_threads_4537)) {\n                        barrier(CLK_LOCAL_MEM_FENCE);\n                    }\n                    if (sle32(skip_threads_4537, sub32(local_tid_4516,\n                                                       mul32(squot32(local_tid_4516,\n                                                                     32),\n                                                             32))) &&\n                        (squot32(local_tid_4516, 32) == 0 &&\n                         slt32(local_tid_4516, segscan_group_sizze_4485))) {\n                        // write result\n                        {\n                            ((volatile __local\n                              int32_t *) scan_arr_mem_4520)[local_tid_4516] =\n                                x_4527;\n                            x_4529 = x_4527;\n                            ((volatile __local\n                              bool *) scan_arr_mem_4522)[local_tid_4516] =\n                                x_4528;\n                            x_4530 = x_4528;\n                        }\n                    }\n                    if (sle32(wave_sizze_4518, skip_threads_4537",
                   ")) {\n                        barrier(CLK_LOCAL_MEM_FENCE);\n                    }\n                    skip_threads_4537 *= 2;\n                }\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // carry-in for every block except the first\n        {\n            if (!(squot32(local_tid_4516, 32) == 0 || !slt32(local_tid_4516,\n                                                             segscan_group_sizze_4485))) {\n                // read operands\n                {\n                    x_4508 = x_4506;\n                    x_4509 = x_4507;\n                    x_4506 = ((__local\n                               int32_t *) scan_arr_mem_4520)[sub32(squot32(local_tid_4516,\n                                                                           32),\n                                                                   1)];\n                    x_4507 = ((__local\n                               bool *) scan_arr_mem_4522)[sub32(squot32(local_tid_4516,\n                                                                        32),\n                                                                1)];\n                }\n                // perform operation\n                {\n                    int32_t res_4510;\n                    bool res_4511;\n                    \n                    if (x_4509) {\n                        bool res_4512 = x_4507 || x_4509;\n                        \n                        res_4510 = x_4508;\n                        res_4511 = res_4512;\n                    } else {\n                        int32_t res_4513 = add32(x_4506, x_4508);\n                        bool res_4514 = x_4507 || x_4509;\n                        \n                        res_4510 = res_4513;\n                        res_4511 = res_4514;\n                    }\n                    x_4506 = res_4510;\n                    x_4507 = res_4511;\n                }\n                // write final result\n                {\n                    ((__local int32_t *) scan_arr_mem_4520)[local_tid_",
                   "4516] =\n                        x_4506;\n                    ((__local bool *) scan_arr_mem_4522)[local_tid_4516] =\n                        x_4507;\n                }\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // restore correct values for first block\n        {\n            if (squot32(local_tid_4516, 32) == 0) {\n                ((__local int32_t *) scan_arr_mem_4520)[local_tid_4516] =\n                    x_4508;\n                ((__local bool *) scan_arr_mem_4522)[local_tid_4516] = x_4509;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // threads in bounds write partial scan result\n        {\n            if (slt32(gtid_4489, n_4462)) {\n                ((__global int32_t *) mem_4497)[gtid_4489] = ((__local\n                                                               int32_t *) scan_arr_mem_4520)[local_tid_4516];\n                ((__global bool *) mem_4499)[gtid_4489] = ((__local\n                                                            bool *) scan_arr_mem_4522)[local_tid_4516];\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n        // first thread reads last element as carry-in for next iteration\n        {\n            bool crosses_segment_4538 = 0;\n            bool should_load_carry_4539 = local_tid_4516 == 0 &&\n                 !crosses_segment_4538;\n            \n            if (should_load_carry_4539) {\n                x_4471 = ((__local\n                           int32_t *) scan_arr_mem_4520)[sub32(segscan_group_sizze_4485,\n                                                               1)];\n                x_4472 = ((__local\n                           bool *) scan_arr_mem_4522)[sub32(segscan_group_sizze_4485,\n                                                            1)];\n            }\n            if (!should_load_carry_4539) {\n                x_4471 = 0;\n                x_4472 = 0;\n            }\n        }\n        barrier(CLK_LOCAL_MEM_FENCE);\n    }\n    \n  error_1:\n    return;\n    #undef segscan_g",
                   "roup_sizze_4485\n}\n__kernel void scan_stage2_4490(__global int *global_failure, __local volatile\n                               int64_t *scan_arr_mem_4565_backing_aligned_0,\n                               __local volatile\n                               int64_t *scan_arr_mem_4563_backing_aligned_1,\n                               int32_t n_4462, int32_t num_groups_4487, __global\n                               unsigned char *mem_4497, __global\n                               unsigned char *mem_4499,\n                               int32_t num_threads_4505)\n{\n    #define segscan_group_sizze_4485 (mainzisegscan_group_sizze_4484)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    __local volatile char *restrict scan_arr_mem_4565_backing_1 =\n                          (__local volatile\n                           char *) scan_arr_mem_4565_backing_aligned_0;\n    __local volatile char *restrict scan_arr_mem_4563_backing_0 =\n                          (__local volatile\n                           char *) scan_arr_mem_4563_backing_aligned_1;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4558;\n    int32_t local_tid_4559;\n    int32_t group_sizze_4562;\n    int32_t wave_sizze_4561;\n    int32_t group_tid_4560;\n    \n    global_tid_4558 = get_global_id(0);\n    local_tid_4559 = get_local_id(0);\n    group_sizze_4562 = get_local_size(0);\n    wave_sizze_4561 = LOCKSTEP_WIDTH;\n    group_tid_4560 = get_group_id(0);\n    \n    int32_t phys_tid_4490 = global_tid_4558;\n    __local char *scan_arr_mem_4563;\n    \n    scan_arr_mem_4563 = (__local char *) scan_arr_mem_4563_backing_0;\n    \n    __local char *scan_arr_mem_4565;\n    \n    scan_arr_mem_4565 = (__local char *) scan_arr_mem_4565_backing_1;\n    \n    int32_t flat_idx_4567 = sub32(mul32(add32(local_tid_4559, 1),\n                                        mul32(segscan_group_sizze_4485,\n                                              squot32(sub32(add32(n_4462,\n               ",
                   "                                                   num_threads_4505),\n                                                            1),\n                                                      num_threads_4505))), 1);\n    int32_t gtid_4489 = flat_idx_4567;\n    \n    // threads in bound read carries; others get neutral element\n    {\n        if (slt32(gtid_4489, n_4462)) {\n            ((__local int32_t *) scan_arr_mem_4563)[local_tid_4559] = ((__global\n                                                                        int32_t *) mem_4497)[gtid_4489];\n            ((__local bool *) scan_arr_mem_4565)[local_tid_4559] = ((__global\n                                                                     bool *) mem_4499)[gtid_4489];\n        } else {\n            ((__local int32_t *) scan_arr_mem_4563)[local_tid_4559] = 0;\n            ((__local bool *) scan_arr_mem_4565)[local_tid_4559] = 0;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    \n    int32_t x_4540;\n    bool x_4541;\n    int32_t x_4542;\n    bool x_4543;\n    int32_t x_4568;\n    bool x_4569;\n    int32_t x_4570;\n    bool x_4571;\n    int32_t skip_threads_4577;\n    \n    if (slt32(local_tid_4559, num_groups_4487)) {\n        x_4542 = ((volatile __local\n                   int32_t *) scan_arr_mem_4563)[local_tid_4559];\n        x_4543 = ((volatile __local bool *) scan_arr_mem_4565)[local_tid_4559];\n        if (sub32(local_tid_4559, mul32(squot32(local_tid_4559, 32), 32)) ==\n            0) {\n            x_4540 = x_4542;\n            x_4541 = x_4543;\n        }\n    }\n    // in-block scan (hopefully no barriers needed)\n    {\n        skip_threads_4577 = 1;\n        while (slt32(skip_threads_4577, 32)) {\n            if (sle32(skip_threads_4577, sub32(local_tid_4559,\n                                               mul32(squot32(local_tid_4559,\n                                                             32), 32))) &&\n                slt32(local_tid_4559, num_groups_4487)) {\n                // read operands\n                {\n          ",
                   "          x_4540 = ((volatile __local\n                               int32_t *) scan_arr_mem_4563)[sub32(local_tid_4559,\n                                                                   skip_threads_4577)];\n                    x_4541 = ((volatile __local\n                               bool *) scan_arr_mem_4565)[sub32(local_tid_4559,\n                                                                skip_threads_4577)];\n                }\n                // perform operation\n                {\n                    int32_t res_4544;\n                    bool res_4545;\n                    \n                    if (x_4543) {\n                        bool res_4546 = x_4541 || x_4543;\n                        \n                        res_4544 = x_4542;\n                        res_4545 = res_4546;\n                    } else {\n                        int32_t res_4547 = add32(x_4540, x_4542);\n                        bool res_4548 = x_4541 || x_4543;\n                        \n                        res_4544 = res_4547;\n                        res_4545 = res_4548;\n                    }\n                    x_4540 = res_4544;\n                    x_4541 = res_4545;\n                }\n            }\n            if (sle32(wave_sizze_4561, skip_threads_4577)) {\n                barrier(CLK_LOCAL_MEM_FENCE);\n            }\n            if (sle32(skip_threads_4577, sub32(local_tid_4559,\n                                               mul32(squot32(local_tid_4559,\n                                                             32), 32))) &&\n                slt32(local_tid_4559, num_groups_4487)) {\n                // write result\n                {\n                    ((volatile __local\n                      int32_t *) scan_arr_mem_4563)[local_tid_4559] = x_4540;\n                    x_4542 = x_4540;\n                    ((volatile __local\n                      bool *) scan_arr_mem_4565)[local_tid_4559] = x_4541;\n                    x_4543 = x_4541;\n                }\n            }\n            if (sle32(wav",
                   "e_sizze_4561, skip_threads_4577)) {\n                barrier(CLK_LOCAL_MEM_FENCE);\n            }\n            skip_threads_4577 *= 2;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // last thread of block 'i' writes its result to offset 'i'\n    {\n        if (sub32(local_tid_4559, mul32(squot32(local_tid_4559, 32), 32)) ==\n            31 && slt32(local_tid_4559, num_groups_4487)) {\n            ((volatile __local\n              int32_t *) scan_arr_mem_4563)[squot32(local_tid_4559, 32)] =\n                x_4540;\n            ((volatile __local\n              bool *) scan_arr_mem_4565)[squot32(local_tid_4559, 32)] = x_4541;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // scan the first block, after which offset 'i' contains carry-in for block 'i+1'\n    {\n        int32_t skip_threads_4578;\n        \n        if (squot32(local_tid_4559, 32) == 0 && slt32(local_tid_4559,\n                                                      num_groups_4487)) {\n            x_4570 = ((volatile __local\n                       int32_t *) scan_arr_mem_4563)[local_tid_4559];\n            x_4571 = ((volatile __local\n                       bool *) scan_arr_mem_4565)[local_tid_4559];\n            if (sub32(local_tid_4559, mul32(squot32(local_tid_4559, 32), 32)) ==\n                0) {\n                x_4568 = x_4570;\n                x_4569 = x_4571;\n            }\n        }\n        // in-block scan (hopefully no barriers needed)\n        {\n            skip_threads_4578 = 1;\n            while (slt32(skip_threads_4578, 32)) {\n                if (sle32(skip_threads_4578, sub32(local_tid_4559,\n                                                   mul32(squot32(local_tid_4559,\n                                                                 32), 32))) &&\n                    (squot32(local_tid_4559, 32) == 0 && slt32(local_tid_4559,\n                                                               num_groups_4487))) {\n                    // read operands\n                    {\n                        x_4568",
                   " = ((volatile __local\n                                   int32_t *) scan_arr_mem_4563)[sub32(local_tid_4559,\n                                                                       skip_threads_4578)];\n                        x_4569 = ((volatile __local\n                                   bool *) scan_arr_mem_4565)[sub32(local_tid_4559,\n                                                                    skip_threads_4578)];\n                    }\n                    // perform operation\n                    {\n                        int32_t res_4572;\n                        bool res_4573;\n                        \n                        if (x_4571) {\n                            bool res_4574 = x_4569 || x_4571;\n                            \n                            res_4572 = x_4570;\n                            res_4573 = res_4574;\n                        } else {\n                            int32_t res_4575 = add32(x_4568, x_4570);\n                            bool res_4576 = x_4569 || x_4571;\n                            \n                            res_4572 = res_4575;\n                            res_4573 = res_4576;\n                        }\n                        x_4568 = res_4572;\n                        x_4569 = res_4573;\n                    }\n                }\n                if (sle32(wave_sizze_4561, skip_threads_4578)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                if (sle32(skip_threads_4578, sub32(local_tid_4559,\n                                                   mul32(squot32(local_tid_4559,\n                                                                 32), 32))) &&\n                    (squot32(local_tid_4559, 32) == 0 && slt32(local_tid_4559,\n                                                               num_groups_4487))) {\n                    // write result\n                    {\n                        ((volatile __local\n                          int32_t *) scan_arr_mem_4563)[local_tid_4559] =\n                ",
                   "            x_4568;\n                        x_4570 = x_4568;\n                        ((volatile __local\n                          bool *) scan_arr_mem_4565)[local_tid_4559] = x_4569;\n                        x_4571 = x_4569;\n                    }\n                }\n                if (sle32(wave_sizze_4561, skip_threads_4578)) {\n                    barrier(CLK_LOCAL_MEM_FENCE);\n                }\n                skip_threads_4578 *= 2;\n            }\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // carry-in for every block except the first\n    {\n        if (!(squot32(local_tid_4559, 32) == 0 || !slt32(local_tid_4559,\n                                                         num_groups_4487))) {\n            // read operands\n            {\n                x_4542 = x_4540;\n                x_4543 = x_4541;\n                x_4540 = ((__local\n                           int32_t *) scan_arr_mem_4563)[sub32(squot32(local_tid_4559,\n                                                                       32), 1)];\n                x_4541 = ((__local\n                           bool *) scan_arr_mem_4565)[sub32(squot32(local_tid_4559,\n                                                                    32), 1)];\n            }\n            // perform operation\n            {\n                int32_t res_4544;\n                bool res_4545;\n                \n                if (x_4543) {\n                    bool res_4546 = x_4541 || x_4543;\n                    \n                    res_4544 = x_4542;\n                    res_4545 = res_4546;\n                } else {\n                    int32_t res_4547 = add32(x_4540, x_4542);\n                    bool res_4548 = x_4541 || x_4543;\n                    \n                    res_4544 = res_4547;\n                    res_4545 = res_4548;\n                }\n                x_4540 = res_4544;\n                x_4541 = res_4545;\n            }\n            // write final result\n            {\n                ((__local int32_t *) scan_arr_mem_4563)[local_",
                   "tid_4559] =\n                    x_4540;\n                ((__local bool *) scan_arr_mem_4565)[local_tid_4559] = x_4541;\n            }\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // restore correct values for first block\n    {\n        if (squot32(local_tid_4559, 32) == 0) {\n            ((__local int32_t *) scan_arr_mem_4563)[local_tid_4559] = x_4542;\n            ((__local bool *) scan_arr_mem_4565)[local_tid_4559] = x_4543;\n        }\n    }\n    barrier(CLK_LOCAL_MEM_FENCE);\n    // threads in bounds write scanned carries\n    {\n        if (slt32(gtid_4489, n_4462)) {\n            ((__global int32_t *) mem_4497)[gtid_4489] = ((__local\n                                                           int32_t *) scan_arr_mem_4563)[local_tid_4559];\n            ((__global bool *) mem_4499)[gtid_4489] = ((__local\n                                                        bool *) scan_arr_mem_4565)[local_tid_4559];\n        }\n    }\n    \n  error_0:\n    return;\n    #undef segscan_group_sizze_4485\n}\n__kernel void scan_stage3_4490(__global int *global_failure, int32_t n_4462,\n                               int32_t num_groups_4487, __global\n                               unsigned char *mem_4497, __global\n                               unsigned char *mem_4499,\n                               int32_t num_threads_4505,\n                               int32_t required_groups_4579)\n{\n    #define segscan_group_sizze_4485 (mainzisegscan_group_sizze_4484)\n    \n    const int block_dim0 = 0;\n    const int block_dim1 = 1;\n    const int block_dim2 = 2;\n    \n    if (*global_failure >= 0)\n        return;\n    \n    int32_t global_tid_4580;\n    int32_t local_tid_4581;\n    int32_t group_sizze_4584;\n    int32_t wave_sizze_4583;\n    int32_t group_tid_4582;\n    \n    global_tid_4580 = get_global_id(0);\n    local_tid_4581 = get_local_id(0);\n    group_sizze_4584 = get_local_size(0);\n    wave_sizze_4583 = LOCKSTEP_WIDTH;\n    group_tid_4582 = get_group_id(0);\n    \n    int32_t phys_tid_4490 = global_tid_4580;\n   ",
                   " int32_t phys_group_id_4585;\n    \n    phys_group_id_4585 = get_group_id(0);\n    for (int32_t i_4586 = 0; i_4586 <\n         squot32(sub32(add32(sub32(required_groups_4579, phys_group_id_4585),\n                             num_groups_4487), 1), num_groups_4487); i_4586++) {\n        int32_t virt_group_id_4587 = add32(phys_group_id_4585, mul32(i_4586,\n                                                                     num_groups_4487));\n        int32_t flat_idx_4588 = add32(mul32(virt_group_id_4587,\n                                            segscan_group_sizze_4485),\n                                      local_tid_4581);\n        int32_t gtid_4489 = flat_idx_4588;\n        int32_t orig_group_4589 = squot32(flat_idx_4588,\n                                          mul32(segscan_group_sizze_4485,\n                                                squot32(sub32(add32(n_4462,\n                                                                    num_threads_4505),\n                                                              1),\n                                                        num_threads_4505)));\n        int32_t carry_in_flat_idx_4590 = sub32(mul32(orig_group_4589,\n                                                     mul32(segscan_group_sizze_4485,\n                                                           squot32(sub32(add32(n_4462,\n                                                                               num_threads_4505),\n                                                                         1),\n                                                                   num_threads_4505))),\n                                               1);\n        \n        if (slt32(gtid_4489, n_4462)) {\n            if (!(orig_group_4589 == 0 || flat_idx_4588 ==\n                  sub32(mul32(add32(orig_group_4589, 1),\n                              mul32(segscan_group_sizze_4485,\n                                    squot32(sub32(add32(n_4462,\n                                             ",
                   "           num_threads_4505), 1),\n                                            num_threads_4505))), 1))) {\n                int32_t x_4549;\n                bool x_4550;\n                int32_t x_4551;\n                bool x_4552;\n                \n                x_4549 = ((__global\n                           int32_t *) mem_4497)[carry_in_flat_idx_4590];\n                x_4550 = ((__global bool *) mem_4499)[carry_in_flat_idx_4590];\n                x_4551 = ((__global int32_t *) mem_4497)[gtid_4489];\n                x_4552 = ((__global bool *) mem_4499)[gtid_4489];\n                \n                int32_t res_4553;\n                bool res_4554;\n                \n                if (x_4552) {\n                    bool res_4555 = x_4550 || x_4552;\n                    \n                    res_4553 = x_4551;\n                    res_4554 = res_4555;\n                } else {\n                    int32_t res_4556 = add32(x_4549, x_4551);\n                    bool res_4557 = x_4550 || x_4552;\n                    \n                    res_4553 = res_4556;\n                    res_4554 = res_4557;\n                }\n                x_4549 = res_4553;\n                x_4550 = res_4554;\n                ((__global int32_t *) mem_4497)[gtid_4489] = x_4549;\n                ((__global bool *) mem_4499)[gtid_4489] = x_4550;\n            }\n        }\n        barrier(CLK_GLOBAL_MEM_FENCE);\n    }\n    \n  error_0:\n    return;\n    #undef segscan_group_sizze_4485\n}\n",
                   NULL};
static const char *size_names[] = {"main.segscan_group_size_4484",
                                   "main.segscan_num_groups_4486"};
static const char *size_vars[] = {"mainzisegscan_group_sizze_4484",
                                  "mainzisegscan_num_groups_4486"};
...
struct sizes {
    size_t mainzisegscan_group_sizze_4484;
    size_t mainzisegscan_num_groups_4486;
} ;
struct futhark_context_config {
...
struct futhark_context {
    ...
    int total_runs;
    long total_runtime;
    cl_kernel scan_stage1_4490;
    cl_kernel scan_stage2_4490;
    cl_kernel scan_stage3_4490;
    ...
    int64_t scan_stage1_4490_total_runtime;
    int scan_stage1_4490_runs;
    int64_t scan_stage2_4490_total_runtime;
    int scan_stage2_4490_runs;
    int64_t scan_stage3_4490_total_runtime;
    int scan_stage3_4490_runs;
                               ...
                               struct futhark_context *ctx)
{
    ...
    ctx->scan_stage1_4490_total_runtime = 0;
    ctx->scan_stage1_4490_runs = 0;
    ctx->scan_stage2_4490_total_runtime = 0;
    ctx->scan_stage2_4490_runs = 0;
    ctx->scan_stage3_4490_total_runtime = 0;
    ctx->scan_stage3_4490_runs = 0;
}
                             ...
                             struct futhark_context *ctx, cl_program prog)
{
    ...
    OPENCL_SUCCEED_OR_RETURN(error);
    {
        ctx->scan_stage1_4490 = clCreateKernel(prog, "scan_stage1_4490",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage1_4490, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
    ...
    {
        ctx->scan_stage2_4490 = clCreateKernel(prog, "scan_stage2_4490",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage2_4490, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "scan_stage2_4490");
    ...
    {
        ctx->scan_stage3_4490 = clCreateKernel(prog, "scan_stage3_4490",
                                               &error);
        OPENCL_SUCCEED_FATAL(error);
        OPENCL_SUCCEED_FATAL(clSetKernelArg(ctx->scan_stage3_4490, 0,
                                            sizeof(cl_mem),
                                            &ctx->global_failure));
        if (ctx->debugging)
            fprintf(stderr, "Created kernel %s.\n", "scan_stage3_4490");
    }
    ctx->sizes.mainzisegscan_group_sizze_4484 = cfg->sizes[0];
    ctx->sizes.mainzisegscan_num_groups_4486 = cfg->sizes[1];
    init_constants(ctx);
...
static int futrts_main(struct futhark_context *ctx,
                       struct memblock_device *out_mem_p_4591,
                       int32_t *out_out_arrsizze_4592,
                       struct memblock_device k_mem_4492,
                       struct memblock_device d_mem_4493, int32_t n_4462,
                       int32_t n_4463);
...
void futhark_debugging_report(struct futhark_context *ctx)
{
    ...
    if (ctx->profiling) {
        ...
        fprintf(stderr,
                "scan_stage1_4490     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage1_4490_runs,
                (long) ctx->scan_stage1_4490_total_runtime /
                (ctx->scan_stage1_4490_runs !=
                 0 ? ctx->scan_stage1_4490_runs : 1),
                (long) ctx->scan_stage1_4490_total_runtime);
        ctx->total_runtime += ctx->scan_stage1_4490_total_runtime;
        ctx->total_runs += ctx->scan_stage1_4490_runs;
        fprintf(stderr,
                "scan_stage2_4490     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage2_4490_runs,
                (long) ctx->scan_stage2_4490_total_runtime /
                (ctx->scan_stage2_4490_runs !=
                 0 ? ctx->scan_stage2_4490_runs : 1),
                (long) ctx->scan_stage2_4490_total_runtime);
        ctx->total_runtime += ctx->scan_stage2_4490_total_runtime;
        ctx->total_runs += ctx->scan_stage2_4490_runs;
        fprintf(stderr,
                "scan_stage3_4490     ran %5d times; avg: %8ldus; total: %8ldus\n",
                ctx->scan_stage3_4490_runs,
                (long) ctx->scan_stage3_4490_total_runtime /
                (ctx->scan_stage3_4490_runs !=
                 0 ? ctx->scan_stage3_4490_runs : 1),
                (long) ctx->scan_stage3_4490_total_runtime);
        ctx->total_runtime += ctx->scan_stage3_4490_total_runtime;
        ctx->total_runs += ctx->scan_stage3_4490_runs;
        if (ctx->profiling)
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       int32_t n_4463)
{
    struct memblock_device out_mem_4502;
    
    out_mem_4502.references = NULL;
    
    int32_t out_arrsizze_4503;
    bool dim_match_4466 = n_4462 == n_4463;
    bool empty_or_match_cert_4467;
    
    if (!dim_match_4466) {
        ctx->error = msgprintf("Error: %s\n\nBacktrace:\n%s",
                               "function arguments of wrong shape",
                               "-> #0  segscan.fut:19:1-61\n");
        if (memblock_unref_device(ctx, &out_mem_4502, "out_mem_4502") != 0)
            return 1;
        return 1;
    }
    
    int64_t n_4482 = sext_i32_i64(n_4462);
    int32_t segscan_group_sizze_4485;
    
    segscan_group_sizze_4485 = ctx->sizes.mainzisegscan_group_sizze_4484;
    
    int32_t num_groups_4487;
    int32_t max_num_groups_4504;
    
    max_num_groups_4504 = ctx->sizes.mainzisegscan_num_groups_4486;
    num_groups_4487 = sext_i64_i32(smax64(1, smin64(squot64(sub64(add64(n_4482,
                                                                        sext_i32_i64(segscan_group_sizze_4485)),
                                                                  1),
                                                            sext_i32_i64(segscan_group_sizze_4485)),
                                                    sext_i32_i64(max_num_groups_4504))));
    
    int64_t bytes_4495 = mul64(4, n_4482);
    struct memblock_device mem_4497;
    
    mem_4497.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4497, bytes_4495, "mem_4497"))
        return 1;
    
    struct memblock_device mem_4499;
    
    mem_4499.references = NULL;
    if (memblock_alloc_device(ctx, &mem_4499, n_4482, "mem_4499"))
        return 1;
    if (ctx->debugging)
        fprintf(stderr, "%s\n", "\n# SegScan");
    
    int32_t num_threads_4505 = mul32(num_groups_4487, segscan_group_sizze_4485);
    
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 1,
                                            mul32((int32_t) sizeof(bool),
                                                  segscan_group_sizze_4485),
                                            NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 2,
                                            mul32((int32_t) sizeof(int32_t),
                                                  segscan_group_sizze_4485),
                                            NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 3,
                                            sizeof(n_4462), &n_4462));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 4,
                                            sizeof(k_mem_4492.mem),
                                            &k_mem_4492.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 5,
                                            sizeof(d_mem_4493.mem),
                                            &d_mem_4493.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 6,
                                            sizeof(mem_4497.mem),
                                            &mem_4497.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 7,
                                            sizeof(mem_4499.mem),
                                            &mem_4499.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage1_4490, 8,
                                            sizeof(num_threads_4505),
                                            &num_threads_4505));
    if (1 * (num_groups_4487 * segscan_group_sizze_4485) != 0) {
        const size_t global_work_sizze_4593[1] = {num_groups_4487 *
                     segscan_group_sizze_4485};
        const size_t local_work_sizze_4597[1] = {segscan_group_sizze_4485};
        int64_t time_start_4594 = 0, time_end_4595 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage1_4490");
            fprintf(stderr, "%zu", global_work_sizze_4593[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4597[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) (0 + mul32((int32_t) sizeof(bool),
                                     segscan_group_sizze_4485) +
                           mul32((int32_t) sizeof(int32_t),
                                 segscan_group_sizze_4485)));
            time_start_4594 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage1_4490,
                                                        1, NULL,
                                                        global_work_sizze_4593,
                                                        local_work_sizze_4597,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage1_4490_runs,
                                                                                                  &ctx->scan_stage1_4490_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4595 = get_wall_time();
            
            long time_diff_4596 = time_end_4595 - time_start_4594;
    ...
    if (ctx->debugging)
        fprintf(stderr, "%s: %llu%c", "elems_per_group",
                (long long) mul32(segscan_group_sizze_4485,
                                  squot32(sub32(add32(n_4462, num_threads_4505),
                                                1), num_threads_4505)), '\n');
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 1,
                                            mul32((int32_t) sizeof(bool),
                                                  num_groups_4487), NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 2,
                                            mul32((int32_t) sizeof(int32_t),
                                                  num_groups_4487), NULL));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 3,
                                            sizeof(n_4462), &n_4462));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 4,
                                            sizeof(num_groups_4487),
                                            &num_groups_4487));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 5,
                                            sizeof(mem_4497.mem),
                                            &mem_4497.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 6,
                                            sizeof(mem_4499.mem),
                                            &mem_4499.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage2_4490, 7,
                                            sizeof(num_threads_4505),
                                            &num_threads_4505));
    if (1 * (1 * num_groups_4487) != 0) {
        const size_t global_work_sizze_4598[1] = {1 * num_groups_4487};
        const size_t local_work_sizze_4602[1] = {num_groups_4487};
        int64_t time_start_4599 = 0, time_end_4600 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage2_4490");
            fprintf(stderr, "%zu", global_work_sizze_4598[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4602[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) (0 + mul32((int32_t) sizeof(bool), num_groups_4487) +
                           mul32((int32_t) sizeof(int32_t), num_groups_4487)));
            time_start_4599 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage2_4490,
                                                        1, NULL,
                                                        global_work_sizze_4598,
                                                        local_work_sizze_4602,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage2_4490_runs,
                                                                                                  &ctx->scan_stage2_4490_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4600 = get_wall_time();
            
            long time_diff_4601 = time_end_4600 - time_start_4599;
    ...
    int32_t required_groups_4579 = squot32(sub32(add32(n_4462,
                                                       segscan_group_sizze_4485),
                                                 1), segscan_group_sizze_4485);
    
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 1,
                                            sizeof(n_4462), &n_4462));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 2,
                                            sizeof(num_groups_4487),
                                            &num_groups_4487));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 3,
                                            sizeof(mem_4497.mem),
                                            &mem_4497.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 4,
                                            sizeof(mem_4499.mem),
                                            &mem_4499.mem));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 5,
                                            sizeof(num_threads_4505),
                                            &num_threads_4505));
    OPENCL_SUCCEED_OR_RETURN(clSetKernelArg(ctx->scan_stage3_4490, 6,
                                            sizeof(required_groups_4579),
                                            &required_groups_4579));
    if (1 * (num_groups_4487 * segscan_group_sizze_4485) != 0) {
        const size_t global_work_sizze_4603[1] = {num_groups_4487 *
                     segscan_group_sizze_4485};
        const size_t local_work_sizze_4607[1] = {segscan_group_sizze_4485};
        int64_t time_start_4604 = 0, time_end_4605 = 0;
        
        if (ctx->debugging) {
            fprintf(stderr, "Launching %s with global work size [",
                    "scan_stage3_4490");
            fprintf(stderr, "%zu", global_work_sizze_4603[0]);
            fprintf(stderr, "] and local work size [");
            fprintf(stderr, "%zu", local_work_sizze_4607[0]);
            fprintf(stderr, "]; local memory parameters sum to %d bytes.\n",
                    (int) 0);
            time_start_4604 = get_wall_time();
        }
        OPENCL_SUCCEED_OR_RETURN(clEnqueueNDRangeKernel(ctx->opencl.queue,
                                                        ctx->scan_stage3_4490,
                                                        1, NULL,
                                                        global_work_sizze_4603,
                                                        local_work_sizze_4607,
                                                        0, NULL,
                                                        ctx->profiling_paused ||
                                                        !ctx->profiling ? NULL : opencl_get_event(&ctx->opencl,
                                                                                                  &ctx->scan_stage3_4490_runs,
                                                                                                  &ctx->scan_stage3_4490_total_runtime)));
        if (ctx->debugging) {
            OPENCL_SUCCEED_FATAL(clFinish(ctx->opencl.queue));
            time_end_4605 = get_wall_time();
            
            long time_diff_4606 = time_end_4605 - time_start_4604;
            
            fprintf(stderr, "kernel %s runtime: %ldus\n", "scan_stage3_4490",
                    time_diff_4606);
        }
    }
    if (memblock_unref_device(ctx, &mem_4499, "mem_4499") != 0)
        return 1;
    out_arrsizze_4503 = n_4462;
    if (memblock_set_device(ctx, &out_mem_4502, &mem_4497, "mem_4497") != 0)
        return 1;
    (*out_mem_p_4591).references = NULL;
    if (memblock_set_device(ctx, &*out_mem_p_4591, &out_mem_4502,
                            "out_mem_4502") != 0)
        return 1;
    *out_out_arrsizze_4592 = out_arrsizze_4503;
    if (memblock_unref_device(ctx, &mem_4499, "mem_4499") != 0)
        return 1;
    if (memblock_unref_device(ctx, &mem_4497, "mem_4497") != 0)
        return 1;
    if (memblock_unref_device(ctx, &out_mem_4502, "out_mem_4502") != 0)
        return 1;
    return 0;
                       ...
                       struct futhark_bool_1d *in1)
{
    ...
    k_mem_4492.references = NULL;
    
    struct memblock_device d_mem_4493;
    
    d_mem_4493.references = NULL;
    ...
    int ret = futrts_main(ctx, &out_mem_4502, &out_arrsizze_4503, k_mem_4492,
                          d_mem_4493, n_4462, n_4463);
    
    if (ret == 0) {
        assert((*out0 =
                (struct futhark_i32_1d *) malloc(sizeof(struct futhark_i32_1d))) !=
            NULL);
        (*out0)->mem = out_mem_4502;
        (*out0)->shape[0] = out_arrsizze_4503;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
