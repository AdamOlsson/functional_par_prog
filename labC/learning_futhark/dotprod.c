...
static void futrts_cli_entry_main(struct futhark_context *ctx)
{
    ...
    struct futhark_i32_1d *read_value_4395;
    int64_t read_shape_4396[1];
    int32_t *read_arr_4397 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4397, read_shape_4396, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 0,
                      "[]", i32_info.type_name, strerror(errno));
    
    struct futhark_i32_1d *read_value_4398;
    int64_t read_shape_4399[1];
    int32_t *read_arr_4400 = NULL;
    
    errno = 0;
    if (read_array(&i32_info, (void **) &read_arr_4400, read_shape_4399, 1) !=
        0)
        futhark_panic(1, "Cannot read input #%d of type %s%s (errno: %s).\n", 1,
                      "[]", i32_info.type_name, strerror(errno));
    if (end_of_input() != 0)
    ...
    if (perform_warmup) {
        ...
        assert((read_value_4395 = futhark_new_i32_1d(ctx, read_arr_4397,
                                                     read_shape_4396[0])) != 0);
        assert((read_value_4398 = futhark_new_i32_1d(ctx, read_arr_4400,
                                                     read_shape_4399[0])) != 0);
        ...
        r = futhark_entry_main(ctx, &result_4401, read_value_4395,
                               read_value_4398);
        if (r != 0)
            futhark_panic(1, "%s", futhark_context_get_error(ctx));
        if (futhark_context_sync(ctx) != 0)
        ...
        assert(futhark_free_i32_1d(ctx, read_value_4395) == 0);
        assert(futhark_free_i32_1d(ctx, read_value_4398) == 0);
        ;
    }
    time_runs = 1;
    // Proper run.
    ...
    write_scalar(stdout, binary_output, &i32_info, &result_4401);
    printf("\n");
    ;
}
...
static int futrts_main(struct futhark_context *ctx,
                       int32_t *out_scalar_out_4394, struct memblock x_mem_4390,
                       struct memblock y_mem_4391, int32_t implz2081U_4374,
                       int32_t implz2081U_4375);
...
static int futrts_main(struct futhark_context *ctx,
                       ...
                       int32_t implz2081U_4375)
{
    int32_t scalar_out_4392;
    bool dim_match_4378 = implz2081U_4374 == implz2081U_4375;
    bool empty_or_match_cert_4379;
    
    if (!dim_match_4378) {
        ctx->error = msgprintf("Error: %s\n\nBacktrace:\n%s",
                               "function arguments of wrong shape",
                               "-> #0  dotprod.fut:1:1-2:31\n");
        return 1;
    }
    
    int32_t res_4381;
    int32_t redout_4388 = 0;
    
    for (int32_t i_4389 = 0; i_4389 < implz2081U_4374; i_4389++) {
        int32_t x_4385 = ((int32_t *) x_mem_4390.mem)[i_4389];
        int32_t x_4386 = ((int32_t *) y_mem_4391.mem)[i_4389];
        int32_t res_4387 = mul32(x_4385, x_4386);
        int32_t res_4384 = add32(res_4387, redout_4388);
        int32_t redout_tmp_4393 = res_4384;
        
        redout_4388 = redout_tmp_4393;
    }
    res_4381 = redout_4388;
    scalar_out_4392 = res_4381;
    *out_scalar_out_4394 = scalar_out_4392;
    return 0;
}
struct futhark_i32_1d {
    struct memblock mem;
...
int futhark_entry_main(struct futhark_context *ctx, int32_t *out0, const
                       ...
                       struct futhark_i32_1d *in1)
{
    struct memblock x_mem_4390;
    
    x_mem_4390.references = NULL;
    
    struct memblock y_mem_4391;
    
    y_mem_4391.references = NULL;
    
    int32_t implz2081U_4374;
    int32_t implz2081U_4375;
    int32_t scalar_out_4392;
    
    lock_lock(&ctx->lock);
    x_mem_4390 = in0->mem;
    implz2081U_4374 = in0->shape[0];
    y_mem_4391 = in1->mem;
    implz2081U_4375 = in1->shape[0];
    
    int ret = futrts_main(ctx, &scalar_out_4392, x_mem_4390, y_mem_4391,
                          implz2081U_4374, implz2081U_4375);
    
    if (ret == 0) {
        *out0 = scalar_out_4392;
    }
    lock_unlock(&ctx->lock);
    return ret;
}
